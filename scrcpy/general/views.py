...
class MobileModelViewSet(ReadOnlyModelViewSet):
    ...
    def screen(self, request, *args, **kwargs):
        kwargs['title'] = self.get_object().device_id or kwargs['device_id']
        kwargs['Screen_id'] = self.get_object().id or kwargs['id']

        if self.get_object().user:
             kwargs['user'] = self.get_object().user or kwargs['user']

        if self.get_object().device_name:
            kwargs['Screen_title'] = self.get_object().device_name or kwargs['device_name']
        else:
            kwargs['Screen_title'] = kwargs['Screen_id']
        if self.get_object().stop_time:
            kwargs['stop_time'] = self.get_object().stop_time or kwargs['stop_time']
        else:
            kwargs['stop_time'] =datetime.datetime(2000, 1, 1, 0, 0, 0)
            
        if request.user.is_superuser:
            kwargs['stop_time']=datetime.datetime(2099, 1, 1, 0, 0, 0)
            print(kwargs['stop_time'])
            return render(request, "general/play.html", kwargs)
        else:
            current_time = time.strftime('%Y-%m-%d', time.localtime(time.time()))
            stop_timeData =kwargs['stop_time']
            stop_time =stop_timeData.strftime('%Y-%m-%d')
            username = request.user;
            intusername = kwargs['user'];
            if current_time<= stop_time and username == intusername :
                return render(request, "general/play.html", kwargs)
            else:
                return HttpResponse("已到期")
    ...
    @action(methods=['get'], detail=True, url_path='listscreen')
    def listscreen(self, request, *args, **kwargs):
        kwargs['title'] = self.get_object().device_id or kwargs['device_id']
        kwargs['Screen_id'] = self.get_object().id or kwargs['id']
        screenconfig = '{"audio": false, "control": true, "video_codec": "h264", "audio_codec": "aac", "cleanup": false, "video_bit_rate": 150000, "max_size": 720,"tunnel_forward": true,"stay_awake": true, "max_fps": 10,"lock_video_orientation": 0}'
        screenJson = json.loads(screenconfig)
        SystemJson = json.loads(self.get_object().config)
        screenJson['video_bit_rate'] = SystemJson['video_bit_rate']
        screenJson['max_size'] = SystemJson['max_size']
        screenJson['max_fps'] = SystemJson['max_fps']
        screenJson['control'] = SystemJson['control']
        url = f"?config={quote(json.dumps(screenJson))}"
        kwargs['url'] = url
        if self.get_object().user:
             kwargs['user'] = self.get_object().user or kwargs['user']
        ...
        if request.user.is_superuser:
            kwargs['stop_time']=datetime.datetime(2099, 1, 1, 0, 0, 0)
            print(kwargs['stop_time'])
            return render(request, "general/listplay.html", kwargs)
        else:
            current_time = time.strftime('%Y-%m-%d', time.localtime(time.time()))
            stop_timeData =kwargs['stop_time']
            stop_time =stop_timeData.strftime('%Y-%m-%d')
    ...
    @action(methods=['post'], detail=True, url_path='filemanager/uploadapk')
    def filemanager_upload(self, request, *args, **kwargs):
        adb_device = adb.AdbDevice(kwargs['device_id'])
        success, error = adb_device.filemanager_upload(request.data['destination'], request._files)
        files = request._files
        for _, file in files.items():
            adb_device.shelcmd("pm install -r " + request.data['destination']+"/"+ file.name)
            print(file.name)
        return Response({"result": {"success": success, "error": error}})
